...
local start = function(t_start)
    ...
    applog.print("local ap ip:", wifi_ap_getip())
    applog.print("local sta ip:", wifi_sta_getip())
    ...
    wsvr:listen(port, function(cli)
        cli:on("connection", function(c)
            applog.print(c:getaddr(), "on ws connection~")
        end)
        ...
        cli:on("disconnection", function(c, s)
            applog.print(c:getaddr(), "on ws disconnect~", s)
            c:on("connection", nil)
            c:on("receive", nil)
