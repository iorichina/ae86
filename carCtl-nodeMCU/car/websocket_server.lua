...
local function sendFile(c, filename)
...
local start = function(t)
    ...
    if wifi.sta.status() ~= wifi.STA_GOTIP then
        applog.print("wait for wifi connection")
        return
    end
    ...
    autoTmr:register(1000, tmr.ALARM_SEMI, function(t)
        stopPwm();
        t:stop();
    end)
